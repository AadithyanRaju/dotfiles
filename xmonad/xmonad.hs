...
myTextEditor     = "code"
myWebBrowser     = "opera" --"google-chrome-stable"
...
myKeys conf@(XConfig {XMonad.modMask = super}) = M.fromList $
    ...
    -- Launch applications
    , ((super,                           xK_n),  spawn myFileManager)
    , ((super,                           xK_e),  spawn myTextEditor)
    , ((super,                           xK_w),  spawn myWebBrowser)
    ...
    , ((super .|. shiftMask,             xK_n),  spawn myRofi_NETWORK)
    , ((super,                           xK_r),  spawn myRofi_ROOT)
    , ((super .|. shiftMask,             xK_b),  spawn myRofi_BLUETOOTH)
    , ((super,                           xK_m),  spawn myRofi_MUSIC)
    , ((super,                           xK_x),  spawn myRofi_POWER)
    , ((super,                           xK_s),  spawn myRofi_SHOTS)
    , ((super .|. shiftMask,             xK_w),  spawn myRofi_WINDOW)
    ...
    , ((super .|. shiftMask,             xK_s),  spawn $ myScreenshot ++ " --area")

    -- Misc
    , ((super,                           xK_p),  spawn myColorPicker)

    -- Lockscreen
    , ((super,                           xK_l),  spawn myLockScreen)
...
myManageHook = composeAll . concat $
    ...
    , [className =? "Alacritty" --> doShift "1" <+> doF (W.greedyView "1")]
    , [className =? "Google-chrome" --> doShift "2" <+> doF (W.greedyView "2")]
    , [className =? "Opera" --> doShift "2" <+> doF (W.greedyView "2")]
    , [className =? "Thunar" --> doShift "3" <+> doF (W.greedyView "3")]
    , [className =? "Code" --> doShift "4" <+> doF (W.greedyView "4")]
    , [className =? "Virt-manager" --> doShift "8" <+> doF (W.greedyView "8")]
    , [className =? "vlc" --> doShift "9" <+> doF (W.greedyView "9")]
    , [className =? "TelegramDesktop" --> doShift "10" <+> doF (W.greedyView "10")]
    ]
    where
